		...
		for (j = 2; j < nyy; j++) {
			for (k = 2, _k = 1; k < nzz; k++, _k++) {
				...
				*/
				...
				// if(myid == 0 && j == 190 &&k == 2 && i ==3) 
				
					// printf("%d\t%2.16f\t%2.16f\t%2.16f\n",i,NR1[i][j][k],NR3[i][j][k],NR5[i][j][k]);
					
				// if(myid == 2 && j == 190 &&k == 2&& i ==26) 
...
// ============================================ //
		...
		itag=510;
		MPI_Sendrecv((void *)&NR1[istart][0][0], icount, MPI_DOUBLE, l_nbr, itag, (void *)&NR1[iend+1][0][0], icount, MPI_DOUBLE, r_nbr, itag, comm, istat);
		itag=520;
		MPI_Sendrecv((void *)&NR2[istart][0][0], icount, MPI_DOUBLE, l_nbr, itag, (void *)&NR2[iend+1][0][0], icount, MPI_DOUBLE, r_nbr, itag, comm, istat);
		itag=530;
		MPI_Sendrecv((void *)&NR3[istart][0][0], icount, MPI_DOUBLE, l_nbr, itag, (void *)&NR3[iend+1][0][0], icount, MPI_DOUBLE, r_nbr, itag, comm, istat);
		itag=540;
		MPI_Sendrecv((void *)&NR4[istart][0][0], icount, MPI_DOUBLE, l_nbr, itag, (void *)&NR4[iend+1][0][0], icount, MPI_DOUBLE, r_nbr, itag, comm, istat);
		itag=550;
		MPI_Sendrecv((void *)&NR5[istart][0][0], icount, MPI_DOUBLE, l_nbr, itag, (void *)&NR5[iend+1][0][0], icount, MPI_DOUBLE, r_nbr, itag, comm, istat);
...
#pragma loop prefetch
			for (mi = istart; mi <= iend; mi++) { 
				...
				*/
				...
				-Bplus12*NL2[i][j-1][k]
				-Bplus13*NL3[i][j-1][k]
				-Bplus14*NL4[i][j-1][k])/deltaET+

					(-Cplus11*NL1[i][j][k-1]
				-Cplus12*NL2[i][j][k-1]
				-Cplus13*NL3[i][j][k-1]
				-Cplus14*NL4[i][j][k-1])/deltaZT;
				...
				-Aplus22*NL2[i-1][j][k])/deltaXI+
					...
					(-Bplus21*NL1[i][j-1][k]
				-Bplus22*NL2[i][j-1][k])/deltaET+

					(-Cplus21*NL1[i][j][k-1]
				-Cplus22*NL2[i][j][k-1])/deltaZT;

				LL3 = (-Aplus31*NL1[i-1][j][k]
				-Aplus33*NL3[i-1][j][k])/deltaXI+

					(-Bplus31*NL1[i][j-1][k]
				-Bplus33*NL3[i][j-1][k])/deltaET+

					(-Cplus31*NL1[i][j][k-1]
				-Cplus33*NL3[i][j][k-1])/deltaZT;

				LL4 = (-Aplus41*NL1[i-1][j][k]
				-Aplus44*NL4[i-1][j][k])/deltaXI+

					(-Bplus41*NL1[i][j-1][k]
				-Bplus44*NL4[i][j-1][k])/deltaET+

					(-Cplus41*NL1[i][j][k-1]
				-Cplus44*NL4[i][j][k-1])/deltaZT;
				...
				-Bplus53*NL3[i][j-1][k]
				-Bplus54*NL4[i][j-1][k]
				-Bplus55*NL5[i][j-1][k])/deltaET+

					(-Cplus51*NL1[i][j][k-1]
				-Cplus52*NL2[i][j][k-1]
				-Cplus53*NL3[i][j][k-1]
				-Cplus54*NL4[i][j][k-1]
				-Cplus55*NL5[i][j][k-1])/deltaZT;
				...
				*/
				...
				NL1[i][j][k] = d11*(MR1[i][j][k]-LL1);
				NL2[i][j][k] = d22*(MR2[i][j][k]-LL2);
				NL3[i][j][k] = d33*(MR3[i][j][k]-LL3);
				NL4[i][j][k] = d44*(MR4[i][j][k]-LL4);
				NL5[i][j][k] = d51*(MR1[i][j][k]-LL1)+d55*(MR5[i][j][k]-LL5);
			
		
			}    // ---- for (mi = 1 ; mi <= Nijk[m]; mi++) ---- //
...
#pragma loop prefetch	
			...
			for (mi = istart; mi <= iend; mi++) { 
				...
				UU1 = (+Amius11*NR1[i+1][j][k]
				+Aplus12*NR2[i+1][j][k]
				+Aplus13*NR3[i+1][j][k]
				+Aplus14*NR4[i+1][j][k])/deltaXI+

					(+Bmius11*NR1[i][j+1][k]
				+Bplus12*NR2[i][j+1][k]
				+Bplus13*NR3[i][j+1][k]
				+Bplus14*NR4[i][j+1][k])/deltaET+

					(+Cmius11*NR1[i][j][k+1]
				+Cplus12*NR2[i][j][k+1]
				+Cplus13*NR3[i][j][k+1]
				+Cplus14*NR4[i][j][k+1])/deltaZT;


				UU2 = (+Aplus21*NR1[i+1][j][k]
				+Amius22*NR2[i+1][j][k])/deltaXI+

					(+Bplus21*NR1[i][j+1][k]
				+Bmius22*NR2[i][j+1][k])/deltaET+

					(+Cplus21*NR1[i][j][k+1]
				+Cmius22*NR2[i][j][k+1])/deltaZT;


				UU3 = (+Aplus31*NR1[i+1][j][k]
				+Amius33*NR3[i+1][j][k])/deltaXI+

					(+Bplus31*NR1[i][j+1][k]
				+Bmius33*NR3[i][j+1][k])/deltaET+

					(+Cplus31*NR1[i][j][k+1]
				+Cmius33*NR3[i][j][k+1])/deltaZT;


				UU4 = (+Aplus41*NR1[i+1][j][k]
				+Amius44*NR4[i+1][j][k])/deltaXI+

					(+Bplus41*NR1[i][j+1][k]
				+Bmius44*NR4[i][j+1][k])/deltaET+

					(+Cplus41*NR1[i][j][k+1]
				+Cmius44*NR4[i][j][k+1])/deltaZT;

				UU5 = (+Aplus51*NR1[i+1][j][k]
				+Aplus52*NR2[i+1][j][k]
				+Aplus53*NR3[i+1][j][k]
				+Aplus54*NR4[i+1][j][k]
				+Amius55*NR5[i+1][j][k])/deltaXI+

					(+Bplus51*NR1[i][j+1][k]
				+Bplus52*NR2[i][j+1][k]
				+Bplus53*NR3[i][j+1][k]
				+Bplus54*NR4[i][j+1][k]
				+Bmius55*NR5[i][j+1][k])/deltaET+

					(+Cplus51*NR1[i][j][k+1]
				+Cplus52*NR2[i][j][k+1]
				+Cplus53*NR3[i][j][k+1]
				+Cplus54*NR4[i][j][k+1]
				+Cmius55*NR5[i][j][k+1])/deltaZT;
				...
				*/
				...
				NR1[i][j][k] = NL1[i][j][k]-d11*UU1;
				NR2[i][j][k] = NL2[i][j][k]-d22*UU2;
				NR3[i][j][k] = NL3[i][j][k]-d33*UU3;
				NR4[i][j][k] = NL4[i][j][k]-d44*UU4;
				NR5[i][j][k] = NL5[i][j][k]-(d51*UU1+d55*UU5);
...
#pragma omp parallel for private(k, rhoold, P, U, V, W, T, VV, rho, Uold, Vold, Wold, VVold, Pold, Told)
...
#pragma omp parallel for private(k) reduction(+:e1,e2,e3,e4,e5)
		...
		for (j = 2; j <= ny; j++) {
			for (k = 2; k <= nz; k++) {
				...
				e1 = e1+Residual1[i][j][k];
				e2 = e2+Residual2[i][j][k];
				e3 = e3+Residual3[i][j][k];
				e4 = e4+Residual4[i][j][k];
				e5 = e5+Residual5[i][j][k];
